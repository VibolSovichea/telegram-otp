...
from app.core.token import generate_otp_code, generate_payload, verify_payload
from app.db.postgres import get_app, log_otp_event, register_app
from app.db.redis import consume_token, get_token, store_token
...
async def register(request: RegisterAppRequest):
    ...
    return RegisterAppResponse(
        ...
        message="App registered successfully. Store your app_secret securely — it will not be shown again.",
    )
...
async def generate_otp(request: GenerateOTPRequest, x_app_secret: str = Header(...)):
    ...
    payload = generate_payload(request.session_id, request.app_id)
    ...
    stored = await store_token(request.session_id, token_data)
    if not stored:
        raise HTTPException(
            status_code=500, detail="Failed to store token. Please try again."
        )
...
async def validate_otp(request: ValidateOTPRequest, x_app_secret: str = Header(...)):
    token_data = await get_token(request.session_id)
    if not token_data:
    ...
    if not parsed:
        ...
        return ValidateOTPResponse(valid=False, message="Session has expired.")
    ...
    if not secrets.compare_digest(expected_code, request.otp_code.upper()):
        ...
        return ValidateOTPResponse(valid=False, message="Invalid OTP code.")

    await consume_token(request.session_id)
